...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, num_heads):
        ...
        # For self-attention: a combined qkv layer
        self.qkv_layer = nn.Linear(d_model, d_model * 3)
        
        # For cross-attention: separate query and key-value layers
        self.q_linear = nn.Linear(d_model, d_model)
        self.kv_linear = nn.Linear(d_model, d_model * 2)
        
        self.out_linear = nn.Linear(d_model, d_model)

    def forward(self, query, mask=None, key_value_input=None):
        batch_size, seq_len_q, _ = query.size()

        if key_value_input is None:
            # Self-attention
            qkv = self.qkv_layer(query)  # (B, seq_len_q, 3*d_model)
            qkv = qkv.reshape(batch_size, seq_len_q, self.num_heads, 3 * self.head_dim)
            qkv = qkv.permute(0, 2, 1, 3)  # (B, heads, seq_len_q, 3*head_dim)
            q, k, v = qkv.chunk(3, dim=-1)
        else:
            # Cross-attention
            q = self.q_linear(query)  # (B, seq_len_q, d_model)
            q = q.reshape(batch_size, seq_len_q, self.num_heads, self.head_dim)
            q = q.permute(0, 2, 1, 3)

            seq_len_kv = key_value_input.size(1)
            kv = self.kv_linear(key_value_input)  # (B, seq_len_kv, 2*d_model)
            kv = kv.reshape(batch_size, seq_len_kv, self.num_heads, 2 * self.head_dim)
            kv = kv.permute(0, 2, 1, 3)
            k, v = kv.chunk(2, dim=-1)

        output, attn_weights = scaled_dot_product_attention(q, k, v, mask)
        output = output.permute(0, 2, 1, 3).reshape(batch_size, seq_len_q, self.d_model)
...
class LayerNormalization(nn.Module):
    ...
    def forward(self, inputs):
        # Normalize over the last dimension only (typical for transformers)
        mean = inputs.mean(dim=-1, keepdim=True)
        variance = ((inputs - mean) ** 2).mean(dim=-1, keepdim=True)
